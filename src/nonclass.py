...
def plot():
    df = utils.load_data(aperture_size=0.3)
    eta_d = 0.8 * 10**(-0.1 * 50 / 10)
    df *= eta_d
    ...
    pre_sel_data = preselection(df, eta_min)

    Q = Qout(alpha_0, r, pre_sel_data.mean(axis=0), (pre_sel_data**2).mean(axis=0), nu)
    QB2 = [mandel_array(mean_Pns(pre_sel_data[t], N=2, alpha_0=alpha_0, r=r)[:-1]) for t in df.columns]
    QB3 = [mandel_array(mean_Pns(pre_sel_data[t], N=3, alpha_0=alpha_0, r=r)[:-1]) for t in df.columns]
    QB5 = [mandel_array(mean_Pns(pre_sel_data[t], N=5, alpha_0=alpha_0, r=r)[:-1]) for t in df.columns]


    E2, E2_e = np.asarray([violation_2d(mean_Pns(pre_sel_data[t], N=2, alpha_0=alpha_0, r=r)[:-1], S=1e6) for t in df.columns]).T
    E3, E3_e = np.asarray([violation_3d(mean_Pns(pre_sel_data[t], N=3, alpha_0=alpha_0, r=r)[:-1], S=1e6) for t in df.columns]).T
    E5, E5_e = np.asarray([violation_5d(mean_Pns(pre_sel_data[t], N=5, alpha_0=alpha_0, r=r)[:-1], S=1e6) for t in df.columns]).T


    dim_scale = 100
    _, ax2 = plt.subplots(1, 1, figsize=(4, 3))
    ax2.plot(*utils.smooth(dim_scale * df.columns, E2, 3), label=f"E2", c="#402602", ls=':')
    ax2.plot(*utils.smooth(dim_scale * df.columns, E3, 3), label=f"E3", c="#542120", ls='--')
    ax2.plot(*utils.smooth(dim_scale * df.columns, E5, 3), label=f"E5", c="#052530", ls='-')

    ax2.fill_between(dim_scale * df.columns, E2 - E2_e, E2 + E2_e, alpha=0.2, color=config.LINE_COLORS[2])
    ax2.fill_between(dim_scale * df.columns, E3 - E3_e, E3 + E3_e, alpha=0.2, color=config.LINE_COLORS[1])
    ax2.fill_between(dim_scale * df.columns, E5 - E5_e, E5 + E5_e, alpha=0.2, color=config.LINE_COLORS[0])

    Q_max = utils.get_intersect(dim_scale * df.columns, Q)
    QB2_max = utils.get_intersect(dim_scale * df.columns, QB2)
    QB3_max = utils.get_intersect(dim_scale * df.columns, QB3)
    QB5_max = utils.get_intersect(dim_scale * df.columns, QB5)

    ylim = ax2.get_ylim()

    ax2.vlines(QB2_max, *ylim, color='k', ls=':', alpha=0.7, lw=1.5)
    ax2.vlines(QB3_max, *ylim, color='k', ls='--', alpha=0.7, lw=1.5)
    ax2.vlines(QB5_max, *ylim, color='k', alpha=0.7, lw=1.5)

    ax2.vlines(Q_max, *ylim, color='k', ls='-.', alpha=0.7, lw=1.5)
    ax2.set_ylim(*ylim)


    ax2.plot(ax2.get_xlim(), [0, 0], c='k', alpha=1, lw=0.5, antialiased=None, snap=True)
    ax2.set_xlim(0, 29)

    ax_t = ax2.twiny()
    ax_t.set_xlim(ax2.get_xlim())
    ax_t.set_xticks([Q_max, QB5_max, QB3_max, QB2_max])
    ax_t.set_xticklabels([r"$Q$", r"$Q_\mathrm{5}$", r" $Q_\mathrm{3}$", r"$Q_\mathrm{2}$"])

    ax2.set_xlabel('Wind-driven shift $s$ [cm]')
    ax2.set_ylabel('Maximum violation')
    plt.tight_layout()
    plt.savefig('plots/6_nonclass.pdf', **config.SAVE_KWARGS)
